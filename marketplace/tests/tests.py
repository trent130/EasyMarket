...
class AddToCartViewTest(TestCase):
    ...
    def test_invalid_form_submission(self):
        ...
        The test checks that the response is a redirect, that the item is not added to the cart, and
        that the view redirects to the home page.
        """
        ...
        response = add_to_cart(request, product_id=self.product.id)
        self.assertEqual(response.status_code, 302)  # Redirect status code
        # Check if item is not added to cart
        self.assertEqual(CartItem.objects.filter(cart=self.cart, product=self.product).count(), 0)
